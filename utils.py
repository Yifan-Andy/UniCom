...
def parse_args():
    ...
    parser.add_argument('--pretrain_dim', type=int, default=None)
    ...
    parser.add_argument('--readout', type=str, default="mean", 
                        help='the readout function')
    parser.add_argument('--n_prompt', type=int, default=20,
                        help='Number of prompt vector shared.')
    parser.add_argument('--alpha', type=float, default=0.1, 
                        help='the value the balance the loss.')
    parser.add_argument('--prompt', type=str, default="GPF-plus", 
                        help='the prompt for downstream part.')
    ...
    parser.add_argument('--n_pos_neg_sample', type=int, default=3, 
                        help='Default sample 3 pos and 3 neg for each query')
    parser.add_argument('--overlapping_thresh', type=float, default=0.5, 
                        help='Threshold for overlapping detection')
    parser.add_argument('--cohesive_subgraphs', action='store_true',
                        help='whether use cohesive enhancement')
    
    # model saving
    parser.add_argument('--save_path', type=str, default='./checkpoints/model/',
                        help='The path for the model to save')
    parser.add_argument('--embedding_path', type=str, default='./checkpoints/pretrain_result/',
...
def load_query_test_n_gt(path, dataset, vec_length):
    # load query and ground truth
    query = []
    file_query = open(path + dataset + '/' + dataset + "_test.query", 'r')
    for line in file_query:
        vec = [0 for i in range(vec_length)]
        line = line.strip()
        line = line.split(" ")
    ...
    gt = []
    file_gt = open(path + dataset + '/' + dataset + "_test.gt", 'r')
    for line in file_gt:
        vec = [0 for i in range(vec_length)]
        line = line.strip()
        line = line.split(" ")
